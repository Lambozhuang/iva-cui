...
- [~] ~~Depending on the task, we should determine if we end the condition based on if the task is finished or not, or if we let the timer run out~~ **Decided: timer-only.** The prompts are now deliberately non-linear one-offs with no task to "finish" (see below), so there is no completion signal to detect — a condition ends on timer expiry (or operator End Run). The `/check_transition` task-finished machinery is now skipped entirely in one-off mode (`ServerInterface`), since it was dead work. Re-introducing task-finished detection would contradict the open-ended conversation design.
- [x] Replace the mic indicator icon from Rec to mic
- [x] **Pre-convo context + Start gate.** After teleport the subject sees a per-task context briefing on the HUD and the conversation is gated closed (mic dead in both pipelines, no error sound) until they press **Start**; the timed window begins on Start. Per-task text in `QoeDeviceClient.kTaskBriefings`; gate is `StudyControls.conversationGateOpen`. Same path for debug + real runs.
...
- [x] **Rewrote all 10 agent system prompts for self-contained, non-linear one-offs** (`transition_prompts_{Shirts,Hotel,Museum,Training}.py`). Each agent keeps its identity/role/voice but now sustains an open-ended free-form chat with no scripted progression, no task to finish, and no handing the user off to another character (the museum volunteers no longer lecture). Shared style/length/non-linear rules are centralised in `conversation_handler.SHARED_STYLE` (appended to every agent prompt) so conversational style is a controlled constant, plus a `SPEAK_MAX_TOKENS` cap on `/speak` as a hard ceiling on reply length — both reduce the LLM+TTS latency variance that would otherwise confound the netem delay measurement.
...
**Polish**
- [ ] Wire `controllerMicButton` on `TrainingSceneController` (currently NULL → Training mic responds to the M key only, not the Quest trigger) (maybe already fixed, test with quest3 and its fine). *(Code-side is correct — the field is read with a null guard and the same `InputActionReference` the zone pipeline uses. This is purely an **Editor step**: assign the A-button `InputActionReference` to `TrainingSceneController.controllerMicButton` in the Inspector, same asset already assigned on `StudyControls`/`SceneControls`.)*

### QoE adaptation — done
- **Non-linear one-off prompts + latency normalization + briefing gate + always-on-top HUD** — see the TODO entries above. Editor step for the HUD fix: add the `AlwaysOnTopCanvas` component to the device HUD canvas root (the GameObject with the `Canvas` + `LazyCameraFollow`); it swaps HUD materials to ZTest-Always overlay shaders so scene geometry can't occlude the floating HUD. The three overlay shaders are in Always Included Shaders so they survive device builds.
- **Filler type is `None` for all agents** (`StudyControls.waitIndicatorType`), so no thinking-canvas/filler-SFX/filler-clip plays during the real network delay — a bare baseline. Set to `Natural`/`Artificial` in the Inspector if studying the wait indicator as a mitigation.
